...
% Jean-Yves Tinevez <tinevez@pasteur.fr> March 2012
    ...
    properties
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%% RRT-specific  properties %%%
        % These should be private
        bestGain = 0;
        bestNodeIdx = 1;
        bestBranchStates = [];
        bestEdgeState = zeros(6,1);
        exactRootState = zeros(6,1);
        iterationCounter = 0;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    end
    ...
    methods
        ...
        function obj = clearTree(obj)
            ...
            obj.bestGain = 0;
            obj.bestNodeIdx = 1;
            obj.bestEdgeState = zeros(6,1);
            obj.exactRootState = zeros(6,1);
            obj.iterationCounter = 0;
        end

        % Get best branch of rrt tree, excluding the root and the child
        % of the root
        function obj = extractBestBranch(obj)
            obj.bestBranchStates = [];
            % Get best node from bestNodeIdx and store it as current node
            currentNode = obj.get(obj.bestNodeIdx); 
            currentNodeIdx = obj.bestNodeIdx;
            % Get idx of parent of currentNode
            currentParentNodeIdx = obj.getparent(currentNodeIdx);
            % As long as the parent of current node is not the root
            while currentParentNodeIdx > 0
                % Add current node state to bestBranchStates, at the top
                obj.bestBranchStates = [currentNode.state.'; obj.bestBranchStates];
                %bestBranchStates = [currentNode; " "; bestBranchStates];
                % currentParentNode becomes currentNode
                currentNode = obj.get(currentParentNodeIdx);
                % Parent of node currentParentNode becomes currentParentNode
                currentParentNodeIdx = obj.getparent(currentParentNodeIdx);
            end
            % At this point, currentNode is the root
            % Eliminate from the best bestBranchStates the state of the 
            % child node of the root, since the child of the root will be
            % the root of the next RRT tree to be built. So we only need
            % the rest of the best branch
            obj.bestEdgeState = obj.bestBranchStates(1,:).';
            obj.exactRootState = obj.bestEdgeState;
            obj.bestBranchStates(1,:) = [];
        end

        function getPathBackToPrevious(obj)
